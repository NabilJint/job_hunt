...
- `insforge()` in browser context is a factory — call it each time, it reads auth from cookies
...
| `GOOGLE_API_KEY`                | agent/extractor.ts     |
...
- `@insforge/sdk` — InsForge client (includes `@insforge/sdk/ssr` for SSR)
